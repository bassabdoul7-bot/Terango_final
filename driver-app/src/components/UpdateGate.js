...
import { View, Text, TouchableOpacity, StyleSheet, Modal, Linking, ActivityIndicator, Image } from 'react-native';
import Constants from 'expo-constants';
import api from '../services/api.service';

// Force-update gate. Calls /api/app-version/driver on launch and on every
// foreground; if the local versionCode is below the server's min, renders
// a non-dismissible full-screen modal. Single CTA opens the Play Store —
// no "Later" by design (the product call is: outdated drivers must update
...
export default function UpdateGate() {
  ...
  async function check() {
    try {
      const res = await api.get('/app-version/driver');
      ...
      // Network failure: don't block. Better to let the driver use a slightly
      // stale build than to brick the app when the API hiccups.
    }
  }
    ...
    <Modal visible animationType="fade" transparent={false} onRequestClose={() => {}}>
      <View style={styles.backdrop}>
        <View style={styles.card}>
          ...
          <Text style={styles.body}>{(config && config.message) || 'Une nouvelle version de TeranGO Pro est disponible. Mettez à jour pour continuer.'}</Text>
