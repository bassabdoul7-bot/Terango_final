...
const RideRequestsScreen = ({ navigation, route }) => {
  ...
  const connectSocket = () => {
    ...
    createAuthSocket().then(function(newSocket) { setSocket(newSocket);
      ...
      newSocket.on('ride-cancelled', (data) => { stopRideAlert(); if (currentRequest && data.rideId === currentRequest.rideId) { setRideRequests(prev => prev.filter(r => r.rideId !== currentRequest.rideId)); var next = rideRequests.find(r => r.rideId !== currentRequest.rideId); setCurrentRequest(next || null); if (offerTimeout) { clearTimeout(offerTimeout); setOfferTimeout(null); } } });
