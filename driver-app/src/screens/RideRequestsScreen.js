...
const RideRequestsScreen = ({ navigation, route }) => {
  ...
  useEffect(() => { var appState = AppState.currentState; var sub = AppState.addEventListener("change", function(next) { if (appState.match(/inactive|background/) && next === "active") { if (socket) { if (!socket.connected) socket.connect(); if (driverId) socket.emit("driver-online", { driverId: driverId, latitude: location?.latitude, longitude: location?.longitude }); } if (driverId && location) { driverService.toggleOnlineStatus(true, location.latitude, location.longitude).catch(function() {}); } /* Recover any in-flight ride offer that the socket missed during background */ driverService.getCurrentOffer().then(function(r) { if (r && r.success && r.offer && !currentRequestRef.current && activeServicesRef.current.rides) { var rd = r.offer; rd._offerType = 'ride'; if (offerTimeoutRef.current) { clearTimeout(offerTimeoutRef.current); } setCurrentRequest(rd); setRideRequests(function(p) { return [...p, rd]; }); var t = setTimeout(function() { handleReject(); }, rd.offerExpiresIn || 15000); offerTimeoutRef.current = t; setOfferTimeout(t); } }).catch(function() {}); } appState = next; }); return () => sub.remove(); }, [socket, driverId, location]);
  const connectSocket = () => {
    if (!driverId) return;
        ...
        {currentRequest ? (
          <View style={styles.requestContent}>
            <View style={styles.requestHeader}>
              ...
              <View style={{alignItems:'flex-end'}}><Text style={styles.fareText}>{(currentRequest.driverEarnings||currentRequest.fare||0).toLocaleString()+' FCFA'}</Text><Text style={styles.fareBreakdown}>{'Commission: '+(currentRequest.platformCommission||0).toLocaleString()+' F'}</Text><Text style={[styles.fareBreakdown,currentRequest.paymentMethod==='wave'&&{color:'#1DC3E1'}]}>{currentRequest.paymentMethod==='wave'?'\uD83C\uDF0A Wave':'\uD83D\uDCB5 Especes'}</Text></View>
            </View>
...
const styles = StyleSheet.create({
  ...
  deliveryHintBanner: { position: 'absolute', top: 170, left: 12, right: 12, flexDirection: 'row', alignItems: 'center', backgroundColor: '#FF9500', borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16, gap: 12, zIndex: 5, elevation: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 6 },
  deliveryHintIcon: { fontSize: 32 },
  deliveryHintTitle: { fontSize: 16, fontFamily: 'LexendDeca_700Bold', color: '#FFFFFF', marginBottom: 3 },
  deliveryHintSub: { fontSize: 13, fontFamily: 'LexendDeca_500Medium', color: '#FFFFFF', opacity: 0.95 },
  deliveryHintArrow: { fontSize: 24, color: '#FFFFFF', fontFamily: 'LexendDeca_700Bold' },
