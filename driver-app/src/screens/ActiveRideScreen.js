...
// CameraView removed — causes crash on ActiveRideScreen mount
...
function ActiveRideScreen(props) {
    ...
    ImagePicker.requestCameraPermissionsAsync().then(function(permission) {
      ...
      ImagePicker.launchCameraAsync({
        ...
        uploadFn.then(function() {
          Alert.alert('Enregistrement sauvegarde', "La video d'urgence a ete envoyee.");
        }).catch(function(err) {
  ...
  function uploadEmergencyVideo(uri, duration) {
    setEmUploading(true);
    var uploadFn = deliveryMode
      ? deliveryService.uploadEmergencyRecording(deliveryId, uri, duration)
      : driverService.uploadEmergencyRecording(rideId, uri, duration);
  ...
  var handleCompleteRide = useCallback(function(){setLoading(true);driverService.completeRide(rideId).then(function(){speakAnnouncement('Course terminee. Le passager a paye '+(ride.fare||0)+' francs. Commission TeranGO '+(ride.platformCommission||0)+' francs. Vos gains '+(ride.driverEarnings||ride.fare||0)+' francs.');if(queuedRide&&queuedRide.accepted){Alert.alert('Course terminée!','Tarif passager: '+(ride.fare?ride.fare.toLocaleString():'0')+' FCFA\nCommission TeranGO (5%): '+(ride.platformCommission?ride.platformCommission.toLocaleString():'0')+' FCFA\nVos gains: '+(ride.driverEarnings?ride.driverEarnings.toLocaleString():'0')+' FCFA\n\nCourse en attente.',[{text:'Commencer',onPress:function(){navigation.replace('ActiveRide',{rideId:queuedRide.rideId,ride:queuedRide});}}]);}else{Alert.alert('Course terminée!','Tarif passager: '+(ride.fare?ride.fare.toLocaleString():'0')+' FCFA\nCommission TeranGO (5%): '+(ride.platformCommission?ride.platformCommission.toLocaleString():'0')+' FCFA\n\nVos gains: '+(ride.driverEarnings?ride.driverEarnings.toLocaleString():'0')+' FCFA',[{text:'OK',onPress:function(){navigation.replace('Home');}}]);}}).catch(function(err){console.error('Complete ride error:',err);Alert.alert('Erreur','Impossible de terminer');}).finally(function(){setLoading(false);});},[rideId,ride,navigation,queuedRide]);
  ...
  var confirmDeliveryStatus = function() {
    ...
    deliveryService.updateDeliveryStatus(deliveryId, pendingDeliveryStatus, deliveryPhoto).then(function() {
      ...
      if (pendingDeliveryStatus === 'delivered') {
        ...
        Alert.alert('Livraison terminée!', 'Gains: ' + (ride.fare ? ride.fare.toLocaleString() : '0') + ' FCFA', [{ text: 'OK', onPress: function() { navigation.replace('Home'); } }]);
    ...
    switch(ride.status){
      ...
      case 'in_progress':
        ...
        if (hasStop && !stopReached) {
          return(<View>{ride.paymentMethod === 'wave' && <View style={styles.waveBanner}><Text style={styles.waveBannerIcon}>{'\uD83C\uDF0A'}</Text><Text style={styles.waveBannerText}>{'Wave: ' + (ride.fare ? ride.fare.toLocaleString() : '0') + ' FCFA'}</Text></View>}{!navigationStarted&&<TouchableOpacity style={styles.navButton} onPress={handleStartNavigation}><Text style={styles.navIcon}>{String.fromCodePoint(0x1F9ED)}</Text><Text style={styles.navText}>{"Naviguer vers l'arret"}</Text></TouchableOpacity>}<GlassButton title={String.fromCodePoint(0x1F4CD) + " Arret atteint"} onPress={function(){confirmThenProceed("arrêt atteint", function(){setStopReached(true);isNearStopRef.current=false;setIsNearStop(false);isNearDestRef.current=false;hapticFiredRef.current=false;setIsNearDestination(false);hasFetchedRoute.current=false;setNavigationStarted(false);speakAnnouncement("Vous etes arrive a l'arret");});}} loading={loading}/></View>);
        }
        return(<View>{ride.paymentMethod === 'wave' && <View style={styles.waveBanner}><Text style={styles.waveBannerIcon}>{'\uD83C\uDF0A'}</Text><Text style={styles.waveBannerText}>{'Wave: ' + (ride.fare ? ride.fare.toLocaleString() : '0') + ' FCFA'}</Text></View>}{!navigationStarted&&<TouchableOpacity style={styles.navButton} onPress={handleStartNavigation}><Text style={styles.navIcon}>{String.fromCodePoint(0x1F9ED)}</Text><Text style={styles.navText}>{"Naviguer vers la destination"}</Text></TouchableOpacity>}<GlassButton title="Terminer la course" onPress={function(){confirmThenProceed("fin de course", handleCompleteRide);}} loading={loading}/></View>);
      default:return null;
    }
  }
    ...
    <View style={styles.container}>
      ...
      {(ride.status==='accepted'||ride.status==='arrived'||ride.status==='in_progress'||ride.status==='at_pickup'||ride.status==='picked_up'||ride.status==='at_dropoff') && (
        <View style={sosDriverStyles.container}>
          ...
          {emUploading && <Text style={sosDriverStyles.uploadingText}>Envoi...</Text>}
        </View>
      )}
      ...
      <SuccessModal visible={showSuccessModal} title="Course annulee" message="La course a ete annulee" onClose={function(){setShowSuccessModal(false);navigation.replace('Home');}}/>
    </View>
  );
}
...
var sosDriverStyles = StyleSheet.create({
  ...
  recordingRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(227,27,35,0.9)', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 8, gap: 8 },
  pulseDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#FF0000' },
  timerText: { fontSize: 13, fontFamily: 'LexendDeca_700Bold', color: '#FFF' },
  stopBtn: { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 4 },
});
