...
function RestaurantMenuScreen(props) {
  ...
  function loadRestaurant() {
    ...
    promise.then(function(response) {
      ...
      else { Alert.alert('Erreur', 'Restaurant non trouvÃ©'); navigation.goBack(); }
      setLoading(false);
    }).catch(function(err) {
  ...
  function getDeliveryAddress() {
    if (currentLocation) {
      var url = 'https://nominatim.openstreetmap.org/reverse?lat=' + currentLocation.latitude + '&lon=' + currentLocation.longitude + '&format=json&accept-language=fr';
  ...
  function handlePlaceOrder() {
    ...
    orderService.createOrder(orderData).then(function(response) {
      ...
      if (response.success) {
        ...
        Alert.alert('Commande envoyÃ©e! ðŸŽ‰', 'Votre commande #' + response.order.orderNumber + ' a Ã©tÃ© envoyÃ©e au restaurant.', [{ text: 'OK', onPress: function() { navigation.goBack(); } }]);
  ...
  var PAYMENT_OPTIONS = [
    { key: 'cash', icon: 'ðŸ’µ', label: 'EspÃ¨ces' },
    { key: 'wave', icon: 'ðŸŒŠ', label: 'Wave' },
    { key: 'orange_money', icon: 'ðŸŸ ', label: 'Orange Money' },
    { key: 'free_money', icon: 'ðŸ’³', label: 'Free Money' },
  ];
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          ...
          <View style={styles.headerMeta}>
            <Text style={styles.headerStars}>{'â­ ' + (restaurant.rating || 0).toFixed(1)}</Text>
        ...
        {restaurant.isOpen ? (
          ...
          <View style={styles.closedBadge}><Text style={styles.closedBadgeText}>FermÃ©</Text></View>
        )}
      </View>
      ...
      <ScrollView style={styles.menuScroll} nestedScrollEnabled={true} showsVerticalScrollIndicator={false}>
        {menuItems.length === 0 ? (
          <View style={styles.emptyMenu}><Text style={styles.emptyIcon}>ðŸ½ï¸</Text><Text style={styles.emptyText}>Aucun plat disponible</Text></View>
        ) : (
              ...
              <View key={item._id} style={styles.menuCard}>
                <View style={styles.menuInfo}>
                  ...
                  <View style={styles.menuBottom}>
                    ...
                    {item.preparationTime ? <Text style={styles.menuTime}>{'â± ' + item.preparationTime + ' min'}</Text> : null}
                  </View>
                </View>
                <View style={styles.menuActions}>
                  {qty > 0 ? (
                    <View style={styles.qtyControl}>
                      <TouchableOpacity style={styles.qtyBtn} onPress={function() { removeFromCart(item._id); }}><Text style={styles.qtyBtnText}>âˆ’</Text></TouchableOpacity>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.cartModal}>
            <View style={styles.cartHeader}>
              ...
              <TouchableOpacity onPress={function() { setShowCart(false); }}><Text style={styles.cartClose}>âœ•</Text></TouchableOpacity>
            </View>
                ...
                return (
                  <View key={item.menuItemId} style={styles.cartItem}>
                    ...
                    <View style={styles.cartItemQty}>
                      <TouchableOpacity style={styles.cartQtyBtn} onPress={function() { removeFromCart(item.menuItemId); }}><Text style={styles.cartQtyBtnText}>âˆ’</Text></TouchableOpacity>
            ...
            <TouchableOpacity style={styles.checkoutBtn} onPress={handleCheckout}>
              <Text style={styles.checkoutBtnText}>{'Commander â€¢ ' + getTotal().toLocaleString() + ' F'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.checkoutModal}>
            <View style={styles.cartHeader}>
              ...
              <TouchableOpacity onPress={function() { setShowCheckout(false); }}><Text style={styles.cartClose}>âœ•</Text></TouchableOpacity>
            </View>
            <ScrollView style={styles.checkoutScroll} nestedScrollEnabled={true}>
              ...
              <Text style={styles.checkoutSection}>Instructions spÃ©ciales</Text>
            ...
            <TouchableOpacity style={[styles.placeOrderBtn, submitting && styles.placeOrderBtnDisabled]} onPress={handlePlaceOrder} disabled={submitting}>
              {submitting ? <ActivityIndicator color={COLORS.darkBg} /> : <Text style={styles.placeOrderText}>{'Confirmer â€¢ ' + getTotal().toLocaleString() + ' F'}</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
