...
const RideSelectionScreen = ({ route, navigation }) => {
  ...
  const getDirections = async () => {
    ...
    try { const url = `http://178.104.14.252:5001/route/v1/driving/${pickup.coordinates.longitude},${pickup.coordinates.latitude};${dropoff.coordinates.longitude},${dropoff.coordinates.latitude}?overview=full&geometries=polyline&steps=true`; const r = await fetch(url); const data = await r.json();
      if (data.code === 'Ok' && data.routes.length > 0) { const route = data.routes[0]; const leg = route.legs[0]; const km = leg.distance / 1000; setRealDistance(km); setRealDuration(Math.round(leg.duration / 60)); setRouteCoordinates(PolylineUtil.decode(route.geometry).map(p => ({ latitude: p[0], longitude: p[1] }))); calculateFares(km, Math.round(leg.duration / 60)); } else { Alert.alert('Erreur', "Impossible de calculer l'itin\u00e9raire"); navigation.goBack(); }
