...
function RestaurantListScreen(props) {
  ...
  var restaurantsState = useState([]);
  var restaurants = restaurantsState[0];
  var setRestaurants = restaurantsState[1];
  ...
  function getFilteredRestaurants() {
    ...
    if (activeFilter !== 'all') {
      filtered = filtered.filter(function(r) {
        return r.cuisine && r.cuisine.some(function(c) {
          return c.toLowerCase().indexOf(activeFilter) !== -1;
        });
      });
    }
    if (search.trim()) {
      ...
      filtered = filtered.filter(function(r) {
        return r.name.toLowerCase().indexOf(q) !== -1 ||
          (r.cuisine && r.cuisine.join(' ').toLowerCase().indexOf(q) !== -1);
      });
    }
    return filtered;
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleWrap}>
          ...
          <Text style={styles.headerSub}>{restaurants.length + ' restaurants disponibles'}</Text>
        </View>
      </View>
      ...
      <View style={styles.filtersWrap}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filtersScroll}>
          {CUISINE_FILTERS.map(function(f) {
      ...
      {loading ? (
        ...
        <View style={styles.emptyWrap}>
          ...
          <Text style={styles.emptySub}>
            {search ? 'Aucun resultat pour "' + search + '"' : 'Les restaurants apparaitront ici bientot!'}
          </Text>
        ...
        <ScrollView style={styles.listWrap} showsVerticalScrollIndicator={false}>
          {filtered.map(function(restaurant) {
            ...
            return (
              <TouchableOpacity
                key={restaurant._id}
                style={[styles.restoCard, !isOpen && styles.restoCardClosed]}
                onPress={function() { handleRestaurantPress(restaurant); }}
                activeOpacity={0.85}
                disabled={!isOpen}
              >
                <View style={styles.restoImageWrap}>
                  {restaurant.coverImage ? (
                    <Image source={{ uri: restaurant.coverImage }} style={styles.restoImage} />
                  ) : (
                    <View style={styles.restoImagePlaceholder}>
                      <Text style={styles.restoImageEmoji}>{'🍽️'}</Text>
                    </View>
                  )}
                  {!isOpen && (
                    <View style={styles.closedOverlay}>
                      <Text style={styles.closedText}>Ferme</Text>
                    </View>
                  )}
                ...
                <View style={styles.restoInfo}>
                  <View style={styles.restoNameRow}>
                    <Text style={styles.restoName} numberOfLines={1}>{restaurant.name}</Text>
                    {isOpen && <View style={styles.openDot} />}
                  </View>

                  <View style={styles.restoMeta}>
                    <Text style={styles.restoStars}>{renderStars(restaurant.rating)}</Text>
                    <Text style={styles.restoRating}>{(restaurant.rating || 0).toFixed(1)}</Text>
                    <Text style={styles.restoDot}>{'•'}</Text>
                    <Text style={styles.restoTime}>{(restaurant.estimatedDeliveryTime || 30) + ' min'}</Text>
                  </View>

                  {restaurant.cuisine && restaurant.cuisine.length > 0 && (
                    <View style={styles.cuisineRow}>
                      {restaurant.cuisine.slice(0, 3).map(function(c, i) {
                        return (
                  ...
                  <View style={styles.restoFooter}>
                    <Text style={styles.restoMin}>
                      {'Min. ' + (restaurant.minimumOrder || 1000).toLocaleString() + ' FCFA'}
                    </Text>
                    <Text style={styles.restoDelivery}>{'🛵 Livraison incluse'}</Text>
                  </View>
                </View>
              </TouchableOpacity>
            );
...
var styles = StyleSheet.create({
  ...
  searchBar: {
    ...
    backgroundColor: MINT_LIGHT, borderRadius: 14, paddingHorizontal: 14,
    borderWidth: 1, borderColor: MINT_BORDER,
  },
  searchIcon: { fontSize: 18, marginRight: 10 },
  searchInput: { flex: 1, color: '#fff', fontSize: 15, paddingVertical: 14 },
  clearIcon: { fontSize: 16, color: 'rgba(255,255,255,0.4)', padding: 4 },
  ...
  filterChip: {
    ...
    borderWidth: 1, borderColor: MINT_BORDER, marginRight: 8,
  },
  filterChipActive: { backgroundColor: 'rgba(252, 209, 22, 0.15)', borderColor: YELLOW },
  filterIcon: { fontSize: 16, marginRight: 6 },
  filterLabel: { fontSize: 13, color: 'rgba(255,255,255,0.5)', fontWeight: '600' },
  filterLabelActive: { color: YELLOW },
  ...
  retryBtn: {
    marginTop: 20, backgroundColor: YELLOW, borderRadius: 14,
    paddingHorizontal: 24, paddingVertical: 12,
  },
  retryBtnText: { fontSize: 15, fontWeight: 'bold', color: '#000' },
  listWrap: { flex: 1, paddingHorizontal: 20, paddingTop: 16 },
  restoCard: {
    backgroundColor: MINT_LIGHT, borderRadius: 20, marginBottom: 16,
    overflow: 'hidden', borderWidth: 1, borderColor: MINT_BORDER,
  },
  restoCardClosed: { opacity: 0.6 },
  restoImageWrap: { height: 140, backgroundColor: 'rgba(179,229,206,0.2)' },
  restoImage: { width: '100%', height: '100%' },
  restoImagePlaceholder: {
  ...
  featuredText: { fontSize: 11, fontWeight: 'bold', color: '#000' },
  restoInfo: { padding: 16 },
  restoNameRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 6 },
  restoName: { fontSize: 18, fontWeight: '700', color: '#fff', flex: 1 },
  openDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#4CD964', marginLeft: 8 },
  restoMeta: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  restoStars: { fontSize: 12, color: YELLOW, marginRight: 4 },
  restoRating: { fontSize: 13, fontWeight: '600', color: '#fff', marginRight: 6 },
  restoDot: { color: 'rgba(255,255,255,0.3)', marginRight: 6 },
  restoTime: { fontSize: 13, color: 'rgba(255,255,255,0.5)' },
  cuisineRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 10 },
  cuisineChip: {
    backgroundColor: 'rgba(179,229,206,0.15)', borderRadius: 8,
    paddingHorizontal: 8, paddingVertical: 3,
  },
  cuisineText: { fontSize: 11, color: 'rgba(255,255,255,0.5)' },
  restoFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  restoMin: { fontSize: 12, color: 'rgba(255,255,255,0.4)' },
