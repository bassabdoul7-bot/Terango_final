...
var ChatScreen = function(props) {
  ...
  var rideId = props.rideId || null;
  var deliveryId = props.deliveryId || null;
  ...
  useEffect(function() {
    ...
    socket.on('chat-history-response', function(history) {
      setMessages(history);
    });
  ...
  function renderMessage(item) {
    var msg = item.item;
    var isMe = msg.senderRole === myRole;
  ...
  function renderEmpty() {
    return (
      <View style={styles.emptyWrap}>
        <Text style={styles.emptyIcon}>{String.fromCodePoint(0x1F4AC)}</Text>
        <Text style={styles.emptyText}>{'Envoyez un message \u00e0 ' + otherName}</Text>
      </View>
    );
  }
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.backBtn} onPress={onClose}>
          <Text style={styles.backArrow}>{String.fromCodePoint(0x2190)}</Text>
        </TouchableOpacity>
      ...
      <FlatList
        ref={flatListRef}
        data={messages}
        keyExtractor={function(item, index) { return item._id || index.toString(); }}
...
var styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: 'rgba(179, 229, 206, 0.08)',
    borderBottomWidth: 1, borderBottomColor: 'rgba(179, 229, 206, 0.15)',
  },
  backBtn: {
  ...
  backArrow: { fontSize: 22, color: '#4CD964' },
  headerInfo: { flex: 1 },
  headerName: { fontSize: 17, fontWeight: '700', color: '#fff' },
  headerSub: { fontSize: 12, color: '#4CD964', marginTop: 2 },
  messageList: { flex: 1 },
  ...
  bubbleMe: { backgroundColor: '#4CD964', borderBottomRightRadius: 4 },
  bubbleOther: { backgroundColor: 'rgba(179, 229, 206, 0.15)', borderBottomLeftRadius: 4 },
  ...
  inputRow: {
    ...
    borderTopWidth: 1, borderTopColor: 'rgba(179, 229, 206, 0.15)',
  },
  input: {
    flex: 1, backgroundColor: 'rgba(255,255,255,0.08)',
    borderRadius: 24, paddingHorizontal: 18, paddingVertical: 12,
    fontSize: 15, color: '#fff', marginRight: 10,
    borderWidth: 1, borderColor: 'rgba(179, 229, 206, 0.15)',
  },
  sendBtn: {
